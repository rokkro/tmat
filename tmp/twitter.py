...
class Setup(): #settings and setup for tweet scraping
    ...
    def follow(self):# https://twitter.com/intent/user?user_id=XXX
        tmp = []
        print("Use http://gettwitterid.com to get a UID.")
        ...
        for i in range(len(tmp)):
            ...
            if tmp[i] == '':  # if blank after spaces removed, dont append to search term list
                continue
            print("Verifying UID...")   #this could break if twitter changes their website
            if requests.get("https://twitter.com/intent/user?user_id=" + tmp[i]).status_code != 200:
                if input("UID '" + tmp[i] + "' not found, attempt to follow anyway? [y/n]").strip() == 'y':
...
if __name__ == '__main__':
    try:
        ...
        if mongo.connected:
            stream(s.term, s.limit(), s.coll_name, s.db_name, s.temp, s.sim, s.lang,['1','2','3'])
        else:
