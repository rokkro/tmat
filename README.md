...
***Modules Needed***: `tweepy` (tweet collection), `nltk` (text analysis), `requests` (image analysis), `pymongo` (mongoDB), and `textstat` (readability analysis). Install these with: `pip install tweepy nltk requests pymongo textstat` or `pip3 install` (Linux + Mac OS). Use `pip install --user tweepy etc` on systems without admin privileges.
