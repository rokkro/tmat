...
def duplicate_find(coll, json_tweet):
    ...
    cursor = cursor.sort([('score', {'$meta': 'textScore'})]).limit(conf['tweet_duplicate_limit']) #sort all by similarity, only give us so many
    for iterator,coll_tweet in enumerate(cursor):  # searching for all tweets' text, removing spaces and punct.
        ...
        if sim_ratio > conf['tweet_similarity_threshold']: # If the text is similar
            #print("\n",iterator)
